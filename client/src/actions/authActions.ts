...
export const registerUser = (credentials: RegisterArgs) => {
    return async (dispatch: Dispatch) => {
        ...
        if ("error" in response) {
            ...
            dispatch(
                showAlert(
                    `Hi 👋 ${response.userDetails.username}. Welcome to Meetflow. I'm Amitkumar, the creator of Meetflow. You have me a as a friend till you invite and add your other friends 😊.`
                )
            );
        }
    };
